(function () {
    ...
    function fallbackImage(post) {
        return post.image_url || 'asset/img/find.png';
    }

    function postCardHTML(post) {
        const image = escapeHTML(fallbackImage(post));
        const href = `detail.html?id=${encodeURIComponent(post.id)}`;
        const name = post.author_name || post.reporter_name || 'Sinh viên USTH';
        const initials = name.trim().charAt(0).toUpperCase() || 'U';

        return `
            <article class="post-card" data-card data-created-at="${escapeHTML(post.created_at || '')}" data-title="${escapeHTML(post.title || '')}" data-location="${escapeHTML(post.location || '')}" data-category="${escapeHTML(post.category || '')}">
                <a href="${href}" class="post-card-image-link"><img src="${image}" alt="${escapeHTML(post.title)}" loading="lazy"></a>
                <div class="post-body">
                    <div class="post-meta-row">
                        <span class="badge ${post.type === 'found' ? 'found' : 'lost'}">${typeLabel(post.type)}</span>
                        <span class="time">${escapeHTML(formatRelativeTime(post.created_at))}</span>
                    </div>
                    <h3><a href="${href}">${escapeHTML(post.title)}</a></h3>
                    <div class="location"><i data-lucide="map-pin"></i><span>${escapeHTML(post.location || 'USTH')}</span></div>
                    <p>${escapeHTML(post.description || '')}</p>
                    <div class="author"><span class="avatar">${escapeHTML(initials)}</span><span>${escapeHTML(name)}</span></div>
                </div>
            </article>
        `;
    }
    ...
    function setupReferenceMobileMenu() {
        ...
        button.addEventListener('click', () => {
            ...
            button.setAttribute('aria-expanded', String(nav.classList.contains('mobile-open')));
        });
    }
